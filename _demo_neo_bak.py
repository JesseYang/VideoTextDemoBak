...
def extract_lines(inputs):
    ...
    for img_idx, each_input in enumerate(inputs):
        ...
        for each_contour in contours:
            ...
            for each_isolated_contour in isolated_contours[:1]:
                ...
                for x in range(canvas_w):
                    ...
                    for y in range(canvas_h):
                        if cv2.pointPolygonTest(each_contour,(x+w_idx_start, y+h_idx_start),False) >= 0:
                            min_coord = y
                            break
                ...
                smoothed = list(np.round(savitzky_golay(np.array(centers), 3, 1)))
                # 从新的中点为基准，放置每一列
...
class Extractor():
    ...
    def from_video(self, video_path):
        ...
        # find out frames that stare at the pages, including run on gpu and postprocess
        self._classify_frames()
        # calculate RUIDU
        self._extract_frames()
        # run detection model and crop the box image
        self._detect_text_area()
        # run segmentation model, without postprocess
        self._segment_lines()
        # dilate, connectivity, alignment, and get the single line image waiting for recognizing
        self._extract_lines()
...
if __name__ == '__main__':
    ...
    video_path_collections = [os.path.join(dirpath,filename) for dirpath, dirnames, filenames in os.walk('test_datasets') for filename in filenames]
    # include all 4 models in this class instance
    # the 4 models are loaded in the initialize method
    ext = Extractor()
    ...
    for i in video_path_collections:
        ...
        target = os.path.join('test_datasets_outputs',filename)
        ...
        for i,j in zip(ext.output_extract_lines, ext.output_recognize_sequences):
            ...
            txt = j[0]
            print(img.shape, txt)
            misc.imsave(os.path.join(target, '{}.png'.format(cnt)), img)
